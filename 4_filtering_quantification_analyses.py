...
radar_height = 0.000  # Modify as km units ASL
...
# Extract the relevant site's geometry
...
for m, m_str in zip(months, months_str):
    ...
    for i, elev in enumerate(ELEVATIONS):
        ...
        # Uploading the PPI metadata (.joblib file; generated by 3_extracting_ppi_metadata.py)
        # ---------------------------------------------------------------------------------------
        print(f'Uploading the PPI metadata of {SITE} {year_month}')
        
        file_name = f"PPI_metadata_{SITE}_{year_month}.joblib"
        output_dir = fr'MY\PATH\TO\PPI_metadata/elev_{elev_predict}/'  #  Modify path
        ...
        # Regional Land-Sea Mask Correction
        # ----------------------------------
        ### NOTE: The following loop applies a specific geometric correction to reclassify 
        # certain areas (e.g., inland regions) that may be misidentified as 'sea' by 
        # the initial mask. This logic is tailored to the current study area's longitudinal layout.
        # Users working with different geographical regions should modify this logic 
        # or comment it out if the standard mask is sufficient.

        corrected_sea_mask = sea_mask.copy()  # Make a copy to avoid modifying the original mask
        ...
        #  Filters per cluster
        # ----------------------
        print("Masking out clusters of non-flocks...")

        # ---------------------------------------------------------------------------------------------------------
        # Set thresholds filters values by month 
        # ---------------------------------------------------------------------------------------------------------
        ### NOTE: 
        # The threshold values below are empirical and specifically tuned for our study 
        # region (e.g., Israel), radar characteristics, and the Flocks Detection Model's performance. 
        # These parameters may not be directly applicable to other geographical areas.
        # If you are applying this to a different region, it is highly recommended to:
        # 1. Perform a sensitivity analysis to determine optimal thresholds.
        # 2. Note: This filtering step primarily targets residual non-biological targets (like clouds). 
        # If an advanced cloud-clearing algorithm is applied prior to prediction, this section may become redundant 
        # and can be commented out or require adjustment.
         # -----------------------------------------------------------------------------------------------------------
        ...
        # ---------------------------
        # Calculating height ranges
        # ---------------------------

        # upload birds_h5_files (.json file; generated in generated by 3_extracting_ppi_metadata.py)
        file_name = f"{SITE}_{year_month}.json"
        output_dir = fr'MY\PATH\TO\bird_h5_files_json/elev_{elev_predict}/'  # Modify as needed
        output_path = os.path.join(output_dir, file_name)
        ...
        birds_data = fp.run_height_range_in_chunks(birds_h5_files, birds_data, radar_height, elev_predict, chunk_size=50)

        end = time.time()
        print(f"run_height_range_in_chunks took {end - start:.2f} seconds")

        del birds_h5_files  # Memory cleanup
        ...
        # Compute bird count
        # --------------------
        print('Compute bird count...')

        # Extract date range
        for index, row in mean_RCSs.iterrows():
            date_range = row['dates range (central 90%)']
            start_str, end_str = date_range.split(" - ")
            mean_RCSs.loc[index, 'start_date'] = datetime.strptime(start_str, "%d.%m")  
            mean_RCSs.loc[index, 'end_date'] = datetime.strptime(end_str, "%d.%m")  

        # Extract month and day from the date
        for data in birds_data:
            date_obj = datetime.strptime(data['date'], "%Y%m%d")  
            month_day = datetime.strptime(f"{date_obj.day}.{date_obj.month}", "%d.%m")  
            ...
            RCS_Tmatix_value = matching_row['RCS_Tmatrix'].values[0]   
        ...
        print(f"Compute bird count completed")
        gc.collect()

        # ------------------------------------------------------------------------------
        # Checkpoint_3: save list of dictionaries (pixle level, only relevant keys)
        # ------------------------------------------------------------------------------
        ...
        output_dir = fr'MY\PATH\TO\Checkpoint_3/elev_{elev_predict}/'  # Modify
        output_path = os.path.join(output_dir, file_name)

        if not os.path.exists(output_dir):
            os.makedirs(output_dir)

        joblib.dump(birds_data_light, output_path)  

        print(f"Saving Checkpoint_3 completed")
        gc.collect()

        # --------------------------------------------------------------------------------------
        # Perform spatial clustering on each scan to identify pixel groupings (potential flocks)
        # --------------------------------------------------------------------------------------
        print(f'Creating clusters for flocks...')
        ...
        for scan in birds_data:
            ...
            for cluster_id in range(1, num_clusters + 1):
                ...
                # Store one metadata row per cluster (flock); 
                cluster_meta.append({
        ...
        print(f"Creating clusters for flocks is completed")
        ...
        cluster_results = []  # Each dictionary contains data for each height-bin × pixel in every cluster.

        for scan, mask, cluster_id in clustered_data:
            cluster_records = fp.gaussian_weighted_cluster(scan, mask, cluster_id)
            cluster_results.extend(cluster_records)  

        print(f"Compute gaussian_weighted_cluster is completed")
        ...
        file_name = f"birds_data_{SITE}_{year_month}.joblib"
        output_dir = fr'MY\PATH\TO\Checkpoint_4/elev_{elev_predict}/'  # Modify
        output_path = os.path.join(output_dir, file_name)
        ...
        print(f"Saving Checkpoint_4 completed")
    ...
    # ----------------------------------------------------------------------------------------------------
    #
    # After all elevations are processed for this month: Combine and clean duplicates across angles
    #
    # ----------------------------------------------------------------------------------------------------
    ...
    # -----------------
    pattern = ( 
        r"MY\PATH\TO\Checkpoint_4"   # Modify
    ...
    # Clean duplicates (pixle level), across all elevations (keep the voxel with the highest bird_count)
    # --------------------------------------------------------------------------------------------------
    df_clean = (
    ...
    # Compute cluster centroids from its pixel coordinates
    ...
    # ----------------------------------------------------------------------------------
    cluster_summary_by_height = cluster_summary_by_height.sort_values(by=['date', 'time', 'height_bin'])
    cluster_summary_by_height = cluster_summary_by_height.rename(columns={"height_bin": "height_bin_asl"})

    # Load DEM
    # ---------
    dem_path = r"MY\PATH\TO\DEM2.tif"  # Modify

    with rasterio.open(dem_path) as dem:
        # Read the elevation data into memory
        dem_array = dem.read(1) 
    
        # Get the row/column indices for all centroids at once
        rows, cols = rasterio.transform.rowcol(
            dem.transform,
            cluster_summary_by_height["centroid_long"].values,
            cluster_summary_by_height["centroid_lat"].values
        )

    # Map the elevation values from the DEM to the dataframe
    cluster_summary_by_height["ground_elev"] = dem_array[rows, cols]

    # Compute height AGL (Above Ground Level)
    cluster_summary_by_height["height_bin_agl_min"] = cluster_summary_by_height["height_bin_asl"] - cluster_summary_by_height["ground_elev"]
    cluster_summary_by_height["height_bin_agl_max"] = (cluster_summary_by_height["height_bin_asl"] + 200) - cluster_summary_by_height["ground_elev"]

    # Filter: keep only clusters where the top of the flock is at least 200m above ground
    flocks_final = cluster_summary_by_height[cluster_summary_by_height["height_bin_agl_max"] > 200].copy()

    # Assign region based on latitude (north or south of the radar location)
    radar_lat = 0  # Modify
    flocks_final['region'] = np.where(flocks_final['centroid_lat'] > radar_lat, 'north', 'south')
    ...
    directory_path = r'MY\PATH\TO\clean_aggregated_csv'  # Modify
    ...
    flocks_final.to_csv(file_path, index=False)
    ...
    print(f"Cluster-level file is saved: {file_path}")
